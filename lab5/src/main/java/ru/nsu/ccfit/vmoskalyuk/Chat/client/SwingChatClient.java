...
public class SwingChatClient extends JFrame {
    ...
    private void connect() {
        try {
            ...
            connection = new ChatClientConnection(host, port, this::handleServerMessage);
            connection.login(name);
            appendSystem("Connected as " + name);
            setConnected(true);
            requestUsers();
        } catch (IOException | NumberFormatException exception) {
            showError(exception.getMessage() == null || exception.getMessage().isBlank()
                    ? "Connection error"
    ...
    private void handleServerMessage(Map<String, Object> message) {
        SwingUtilities.invokeLater(() -> {
            Map<String, Object> error = JsonUtil.object(message.get("error"));
            if (!JsonUtil.string(error.get("message")).isBlank()) {
                appendSystem("Error: " + JsonUtil.string(error.get("message")));
                setConnected(false);
                return;
            }
            Map<String, Object> success = JsonUtil.object(message.get("success"));
            if (success.containsKey("listusers")) {
                showUsers(success.get("listusers"));
                return;
            }
            Map<String, Object> event = JsonUtil.object(message.get("event"));
            String eventName = JsonUtil.string(event.get("name"));
            switch (eventName) {
                case "message" -> appendMessage(
                        JsonUtil.string(event.get("user")),
                        JsonUtil.string(event.get("message")),
                        JsonUtil.string(event.get("time")));
                case "userlogin" -> {
                    appendSystem(JsonUtil.string(event.get("user")) + " joined chat");
                    requestUsers();
                }
                case "userlogout" -> {
                    appendSystem(JsonUtil.string(event.get("user")) + " left chat");
                    requestUsers();
                }
                default -> {
                    if (!success.isEmpty()) appendSystem("Command completed");
                }
            }
        });
    }

    private void showUsers(Object usersValue) {
        usersModel.clear();
        if (!(usersValue instanceof List<?> users)) return;
        for (Object item : users) {
            Map<String, Object> user = JsonUtil.object(item);
            String name = JsonUtil.string(user.get("name"));
            String type = JsonUtil.string(user.get("type"));
            if (!name.isBlank())
                usersModel.addElement(type.isBlank() ? "🌸 " + name : "🌸 " + name + " (" + type + ")");
        }
    }
    ...
    private void showError(String message) {
        ...
        JOptionPane.showMessageDialog(this, text, "💔 Chat Error", JOptionPane.ERROR_MESSAGE);
    }
    ...
    private static JScrollPane pinkScrollPane(java.awt.Component view) {
